...
import controller.ReviewListCT;
import entity.UserET;

public class ReviewListUI extends JFrame{
	private ReviewListCT controller;
	private UserET user;
	private int selectedBugID;
	private Object[][] allBugs;
	private String[] colNames = {"BugID", "BugType", "Status", "Description"};
	
	public ReviewListUI(UserET obj) {
		user = obj;
		controller = new ReviewListCT();
		allBugs = controller.getRevList();
	...
	private void initialize() {
		...
		JMenu mnGoto;
		JMenuItem mntmBugList,mntmBugSearch,mntmReviewBugs,mntmLogout;
		...
		lblTitle = new JLabel("Bugs to review");
		lblTitle.setFont(new Font("Tahoma", Font.BOLD, 24));
		DetailPanel.add(lblTitle);
		
		lblBugsFound = new JLabel(Integer.toString(allBugs.length) + " bugs to review.");
	...
	private void rightClickMenu(JPopupMenu pm) {
		...
		ReviewBug = new JMenuItem("Review Bug");
		ReviewBug.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
